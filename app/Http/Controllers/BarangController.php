...
class BarangController extends Controller
{
    ...
    {
        $barangs = Barang::with('satuans')->get();
    ...
    public function storeKomputer(Request $request)
    {
        ...
        $messages = [
            ...
            'finalData.*.harga.required' => 'Harga harus diisi.',
        ...
        foreach ($finalData as $data) {
            ...
            if ($barang) {
                $errors = [
                    'barang_sudah ada' => ['Barang ' . $barang->kode . ' sudah ada']
                ];
            ...
            $newBarang = Barang::create([
                ...
                'manual' =>  $data['manual'],
                'harga' => intval(str_replace(".", "", $data['harga'],)),
                'gambar' => $data['src'],
            ]);
        }
    ...
    public function storeHp(Request $request)
    {
        ...
        // UNTUK GAMBAR
        if ($request->src) {
            if ($request->src != "https://static.vecteezy.com/system/resources/previews/005/337/799/original/icon-image-not-found-free-vector.jpg") {
